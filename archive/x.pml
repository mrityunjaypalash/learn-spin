            ...
            ((m==S_SEARCH));
             {
               ...
               printf('\\nCP(%d): Search: loc = %d, val = %d, nextloc = %d, nextval = %d',id,l,k,next,nextval);
               if
               ::
                  ((nextval<k));
                  lnext = next;
            ...
            ((m==S_INSERT));
             {
               ...
               ::
                  ((list[next].value==k));
                  printf('\\nCP(%d): %d already exists at index %d. Skipping the insert',id,k);
               ...
               ::
                  ...
                  d_step {
                     printf('(%d)Critical Value (before): %d, a: %d, b: %d,  c:%d',_pid,critical_val,list[l].value,list[next].value,k);
...
proctype HubController()
{
   ...
   ::
      (((h_state.proc[0].isOver==1)&&(h_state.proc[0].isOver==1)));
      printf('We should exit - both processes are done!');
      goto :b1;
