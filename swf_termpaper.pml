...
/*
	...
	This is a clever way to capture the state of the array in a single variable critical_val. So 
	safety is checked just by checking that this variable stays positive. 
	...
	We keep a counter insert_counter which holds the # of inserts done by each process. For this to be
	live, the count should be > 0 for each of them. In fact, it should be same as the number of inserts
	they attempted.
...
chan ch_PtoH = [MQLENGTH] of {byte, Pstate}
chan ch_HtoP[2] = [MQLENGTH] of {byte, Pinput}

byte insert_counter[2]; // Counts how many inserts have been done by each process

inline do_insert(L_val, loc) {
    
	next = list[loc].next
    
	// Count this as an insert, even if this was a duplicate and you will drop it. 
	insert_counter[id-1] ++;
	/* 
...
/*
	...
	If a process is ready to insert, no other process can move if they are also on the same node. If 
	the other node is searching, then make it stop (c = 0) - this is not in the paper. 
	If the other is ready to insert, then let the process with the bigger number in hand insert so that next insert by the other process
	doesn't break the sequence - this is already given by the paper
...
inline hub_nextstate() {
	...
	if
	:: (m1 == S_INSERT) && (m2 == S_INSERT) && (l1 == l2) -> // Let only the right one move forward
		if
		:: (k1 < k2) ->
			c1 = 0
			c2 = 1
	...
	:: (m1 == S_INSERT) && (m2 == S_SEARCH) && (l1 == l2) -> // P2 has to wait
		c1 = 1
		c2 = 0

	:: (m1 == S_SEARCH) && (m2 == S_INSERT) && (l1 == l2) -> // P1 has to wait
		c1 = 0
		c2 = 1
...
proctype HubController(byte id1; byte id2) {
	...
	do 
		:: ch_PtoH ?? 1, h_input.proc[0] ->
			//copy_to_hstate(0, p_state1)
			if
			:: ch_PtoH ?? 2, h_input.proc[1] ->
...
active proctype parent() {
	...
	run ChildProcess(1, ch_HtoP[0]);
	run ChildProcess(2, ch_HtoP[1]);
